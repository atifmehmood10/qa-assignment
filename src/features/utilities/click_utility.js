...
class ClickUtility {
  ...
  async clickElementByEval (page, element) {
    try {
      await page.waitForSelector(element)
  ...
  async getAttributeValueUsingCss (page, element, attribute) {
    await page.waitForFunction(selector => !!document.querySelector(selector), {}, element);
    return await page.$$eval(element, el => el.map(x => x.getAttribute(attribute)));
  }
